...
#../- > means , from the current location, one position upwards .
...
Test Setup    Start_Browser_And_Maximize
Test Teardown    Close Browser Window
Documentation    This file has te testcases which allows user to get the data from json file and input those data to the form,
...    which needs to be filled in the website
...    We have created user defined .py methods and created keywords using the python functions and used in the file
Library    SeleniumLibrary
...
Enter RegistrationForm Using JsonFile
    ...
    press keys    css:[value='Sign up']    RETURN
